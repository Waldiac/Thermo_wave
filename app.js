...
function bind() {
  ...
  $('mbAdd').addEventListener('click', addMaterialBuilderItem);
  $('mbUpdate').addEventListener('click', updateMaterialBuilderItem);
  $('mbCancel').addEventListener('click', clearMaterialBuilderForm);
  $('mbClearForm').addEventListener('click', clearMaterialBuilderForm);
  $('mbImportToDb').addEventListener('click', importBuilderListToMaterialDb);
  $('mbExport').addEventListener('click', exportMaterialBuilderList);
  $('mbImport').addEventListener('change', e => importMaterialBuilderList(e.target.files[0]));
  $('mbClearList').addEventListener('click', () => { materialBuilderList = []; saveMaterialBuilderList(); renderMaterialBuilder(); clearMaterialBuilderForm(); });
  ...
  $('importJsonTop').addEventListener('change', e => importJson(e.target.files[0]));
  $('exportMaterialsTop').addEventListener('click', exportMaterials);
  $('importMaterialsTop').addEventListener('change', e => importMaterials(e.target.files[0]));
  $('printPage').addEventListener('click', () => window.print());
  $('openManualTop').addEventListener('click', scrollToManual);
  $('printManual').addEventListener('click', printManualOnly);
  document.querySelectorAll('.manual-tab').forEach(btn => btn.addEventListener('click', () => showManualTab(btn.dataset.manualTab)));
  ...
  $('scoreModeSelect').addEventListener('change', e => { currentScoreMode = e.target.value || 'balanced'; localStorage.setItem('stw_score_mode', currentScoreMode); renderComparisonGraphic(); });
