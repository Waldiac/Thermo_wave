...
const I18N = {
  de: {
    mainTitle:'Sommerliches Verhalten von Konstruktionen', subtitle:'Dynamische thermische Kennwerte nach EN ISO 13786, berechnet mit dem kalibrierten Matrixverfahren der Excel-Datei.', language:'Sprache', printPdf:'Drucken / PDF',
    ...
    selectMaterial:'Material aus Datenbank wählen…', noSaved:'Noch keine gespeicherten Konstruktionen', selectSaved:'Gespeicherte Konstruktion wählen…', noLayers:'Noch keine Schichten vorhanden. Beginnen Sie mit der inneren Schicht.', edit:'Bearbeiten', remove:'Entfernen', load:'Laden',
  ...
  en: {
    mainTitle:'Summer Thermal Behaviour of Constructions', subtitle:'Dynamic thermal parameters according to EN ISO 13786, based on the calibrated Excel matrix method.', language:'Language', printPdf:'Print / PDF',
...
function renderLayers() {
  ...
  tbody.innerHTML = state.layers.map((l, i) => `
    <tr>
      ...
      <td><button class="small" data-edit="${i}">${t('edit')}</button> <button class="small danger" data-remove="${i}">${t('remove')}</button></td>
    </tr>`).join('');
