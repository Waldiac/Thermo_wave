...
function materialBuilderStorageKey() { return 'stw_material_builder_list'; }
function loadMaterialBuilderList() {
  try {
    materialBuilderList = JSON.parse(localStorage.getItem(materialBuilderStorageKey()) || '[]') || [];
  } catch {
    materialBuilderList = [];
  }
}
function saveMaterialBuilderList() {
  localStorage.setItem(materialBuilderStorageKey(), JSON.stringify(materialBuilderList));
}
...
function materialBuilderItemFromForm() {
  ...
  const category = mbVal('mbCategory') || 'Eigene Materialien';
  ...
  const mu = mbVal('mbMu');
  const slug = (name_en || name_de).toLowerCase().replace(/[^a-z0-9äöüß]+/g, '-').replace(/^-|-$/g, '');
  return {
    id: `user-${slug || crypto.randomUUID()}`,
...
function clearMaterialBuilderForm() {
  ...
  $('mbAdd').disabled = false;
  $('mbUpdate').disabled = true;
  $('mbCancel').disabled = true;
}
...
function addMaterialBuilderItem() {
  ...
  materialBuilderList.push(item);
  saveMaterialBuilderList();
  renderMaterialBuilder();
...
function updateMaterialBuilderItem() {
  ...
  const item = materialBuilderItemFromForm();
  if (!validateMaterialBuilderItem(item)) { alert(t('mbInvalid')); return; }
  materialBuilderList[materialBuilderEditIndex] = item;
  saveMaterialBuilderList();
  renderMaterialBuilder();
  clearMaterialBuilderForm();
}
function renderMaterialBuilder() {
  const body = $('mbBody');
  if (!body) return;
  if (!materialBuilderList.length) {
    body.innerHTML = `<tr><td colspan="10" class="empty">${t('mbNoRows')}</td></tr>`;
    return;
  }
  body.innerHTML = materialBuilderList.map((m, i) => `<tr>
    <td>${i + 1}</td>
    <td>${esc(m.name_de || m.name_en)}</td>
    <td>${esc(m.category || '')}</td>
    <td>${fmtFlex(m.lambda, 4)}</td>
    <td>${fmtFlex(m.density, 1)}</td>
    <td>${fmtFlex(m.specificHeat, 1)}</td>
    <td>${fmtFlex(m.defaultThickness, 4)}</td>
    <td>${esc(m.mu || '')}</td>
    <td>${esc(m.source || '')}</td>
  ...
  body.querySelectorAll('[data-mb-edit]').forEach(btn => btn.addEventListener('click', () => {
    ...
    fillMaterialBuilderForm(materialBuilderList[materialBuilderEditIndex]);
    $('mbAdd').disabled = true;
    $('mbUpdate').disabled = false;
    $('mbCancel').disabled = false;
  ...
  body.querySelectorAll('[data-mb-del]').forEach(btn => btn.addEventListener('click', () => {
    materialBuilderList.splice(Number(btn.dataset.mbDel), 1);
    saveMaterialBuilderList();
    renderMaterialBuilder();
  }));
}
function importBuilderListToMaterialDb() {
  if (!materialBuilderList.length) return;
  materialBuilderList.forEach(src => {
    const material = {
      id: src.id || crypto.randomUUID(),
      name_de: src.name_de || src.name_en,
      name_en: src.name_en || src.name_de,
      category: src.category || 'User materials',
      lambda: src.lambda,
      density: src.density,
      specificHeat: src.specificHeat,
      defaultThickness: src.defaultThickness || 0.01,
      mu: src.mu || '',
      source: src.source || t('mbImportDbNote')
    };
    const idx = materialDb.findIndex(m => m.id === material.id || (m.name_de || '').toLowerCase() === (material.name_de || '').toLowerCase());
    if (idx >= 0) materialDb[idx] = { ...materialDb[idx], ...material };
    else materialDb.push(material);
  });
  saveMaterialsLocal();
  renderMaterialSelect();
...
function exportMaterialBuilderList() {
  const payload = {
    meta: { tool: 'ThermoWave material builder', version: TOOL_VERSION, exported: new Date().toISOString() },
    materials: materialBuilderList
  };
  const blob = new Blob([JSON.stringify(payload, null, 2)], { type: 'application/json' });
  const a = document.createElement('a');
  a.href = URL.createObjectURL(blob);
  a.download = 'thermowave-material-builder-list.json';
  a.click();
...
function importMaterialBuilderList(file) {
  ...
  reader.onload = () => {
    const data = JSON.parse(reader.result);
    const incoming = data.materials || data;
    if (!Array.isArray(incoming)) return;
    materialBuilderList = incoming.map(m => ({
      id: m.id || crypto.randomUUID(),
      name_de: m.name_de || m.name || m.name_en || '',
      name_en: m.name_en || m.name || m.name_de || '',
      category: m.category || 'Eigene Materialien',
      lambda: Number(m.lambda),
      density: Number(m.density),
      specificHeat: Number(m.specificHeat),
      defaultThickness: Number(m.defaultThickness || m.thickness || 0.01),
      mu: m.mu || '',
      source: m.source || ''
    })).filter(validateMaterialBuilderItem);
    saveMaterialBuilderList();
    renderMaterialBuilder();
  };
  reader.readAsText(file);
...
function bind() {
  ...
  on('mbImportToDb', 'click', importBuilderListToMaterialDb);
  on('mbExport', 'click', exportMaterialBuilderList);
  on('mbImport', 'change', e => importMaterialBuilderList(e.target.files[0]));
  on('mbClearList', 'click', () => { materialBuilderList = []; saveMaterialBuilderList(); renderMaterialBuilder(); clearMaterialBuilderForm(); });
