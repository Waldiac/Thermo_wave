...
function getSavedLibrary() {
  ...
  if (lib) { try { return JSON.parse(lib); } catch { return []; } }
...
function getComparisonSet() {
  try { return JSON.parse(localStorage.getItem('stw_comparison') || '[]'); } catch { return []; }
}
...
function makeConstructionItem() {
  ...
  return {
    ...
    results: { U0: res.U0, Y12: res.Y12.value, shift: res.Y12.phase.dynamic, f: res.decrement, damping: res.damping, chi1: res.chi1, chi2: res.chi2, thickness: res.totalThickness, heatCapacity: res.totalHeatCapacity }
  };
}
...
function renderComparisonInterpretation(saved, best) {
  ...
  box.innerHTML = `
    ...
    <ul>
      ...
      <li><strong>Latest heat-wave arrival:</strong> ${esc(by('shift', best.maxShift))} has the largest time shift (${fmt(best.maxShift, 2)} h). This can move the peak from afternoon into evening/night, depending on the construction.</li>
...
function renderComparison() {
  ...
  </tr>`).join('') : '<tr><td colspan="12" class="empty">No constructions in the comparison yet. Save or load a construction and click “Add/update in comparison”.</td></tr>';
...
function plainRankList(saved) {
  return saved.map((x, i) => {
    ...
    return `<tr><td><strong>${constructionLabel(i)}</strong></td><td>${esc(x.name)}</td><td>${fmt(qDyn, 1)} W/m²</td><td>${fmt(x.results.f * 100, 0)}%</td><td>${fmt(x.results.shift, 1)} h</td><td>${fmt(storage, 2)} kWh</td></tr>`;
  }).join('');
}
function summerScore(saved) {
  ...
  const ranges = { f: vals('f'), Y12: vals('Y12'), shift: vals('shift'), chi1: vals('chi1') };
  ...
  return saved.map((x, i) => ({
    index: i,
    item: x,
    score: 0.35 * normLow(x.results.f, ranges.f) + 0.25 * normLow(x.results.Y12, ranges.Y12) + 0.20 * normHigh(x.results.shift, ranges.shift) + 0.20 * normHigh(x.results.chi1, ranges.chi1)
...
function generateComparativeReport() {
  ...
  box.innerHTML = `
    ...
    <p>Assumption for interpretation only: daily mean outdoor temperature 24 °C and afternoon peak 36 °C. This corresponds to an outdoor temperature amplitude of 12 K. The table below estimates what this periodic heat wave means for each construction.</p>
    ...
    <ul>
      ...
      ${compareSentence(maxShift, 'largest time shift', 'The heat-flow peak arrives latest. This is useful when night ventilation can remove the delayed heat.', ' h', 2)}
    ...
    <p><strong>Best combined tendency:</strong> Construction ${constructionLabel(ranking[0].index)} (${esc(ranking[0].item.name)}). This ranking combines low f, low |Y₁₂|, large time shift and high χ₁. It is a practical reading aid, not an EN ISO 13786 rating class.</p>
    <p><strong>How to read this:</strong> A wall with low f and low |Y₁₂| keeps the outdoor heat wave small on the room side. A high χ₁ helps absorb short internal or solar gains at the inner surface. A useful time shift is most valuable when the delayed heat can be removed by evening or night ventilation.</p>`;
}
