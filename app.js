...
function generatePairComparison() {
  ...
  const rows = [
    pairCompareLine('U₀', rA.U0, rB.U0, 4, 'low', t('pairU0Explain'), ' W/(m²K)'),
    pairCompareLine('|Y₁₂|', rA.Y12, rB.Y12, 4, 'low', t('pairY12Explain'), ' W/(m²K)'),
    pairCompareLine(t('delayH'), rA.delay, rB.delay, 2, 'high', t('pairDelayExplain'), ' h'),
    pairCompareLine('f', rA.f, rB.f, 4, 'low', t('pairFExplain')),
    pairCompareLine('1/f', rA.damping, rB.damping, 2, 'high', t('pairDampingExplain')),
    pairCompareLine('χ₁', rA.chi1, rB.chi1, 1, 'high', t('pairChi1Explain'), ' kJ/(m²K)'),
    pairCompareLine('χ₂', rA.chi2, rB.chi2, 1, 'high', t('pairChi2Explain'), ' kJ/(m²K)')
  ].join('');
  box.innerHTML = `
    ...
    <table class="pair-table"><thead><tr><th>${t('pairIndicator')}</th><th>${t('pairValueA')}</th><th>${t('pairValueB')}</th><th>${t('pairAssessment')}</th></tr></thead><tbody>${rows}</tbody></table>
    <div class="pair-conclusion">
      <h4>${t('pairConclusionTitle')}</h4>
      <p>${t('pairConclusionDynamic', {name: dynName})}</p>
      <p>${t('pairConclusionStorage')} ${storageName} ${currentLang === 'de' ? 'hat hier den höheren χ₁-Wert.' : 'has the higher χ₁ value here.'}</p>
