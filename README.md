...
Improves language consistency for layer names. Layers created from the material database now store both German and English material names where available. Layer tables, result check tables and path detail tables display the language-appropriate name when switching DE/EN. Existing layers are also translated where they match a material database entry.


## Version 5.6

Fixes manual editing of bilingual layer names. When a user changes a layer name after editing a material-based layer, the manually entered name now takes precedence over the stored DE/EN material name. This allows adding text such as '36.5 cm' and later removing it again without the old name reappearing.
...
Adds `constructions.json` as a server-side example-construction database. Example walls can be selected and loaded in section 2. User changes are still saved only locally in the browser via localStorage; server files are read-only for users. Required runtime files now include `constructions.json` in addition to `materials.json`.


## Version 6.2

Clarifies the difference between user-saved local constructions and server-provided example constructions. The UI now labels the local browser library and the fixed examples from `constructions.json` more clearly. Calibration examples are moved into a separate collapsible test-case area.
